...
if (-not $SkipMigration) {
    ...
    $migrationFile = "flask-api/migrations/add_missing_columns.sql"
    
    if (Test-Path $migrationFile) {
        Write-Host "Migration file found: $migrationFile" -ForegroundColor Green
        Write-Host ""
        Write-Host "⚠️  You will be prompted for database password" -ForegroundColor Yellow
        Write-Host "Password: QuoTrade2024!SecureDB" -ForegroundColor Cyan
        Write-Host ""
        ...
        if ($psqlExists) {
            ...
            if ($LASTEXITCODE -eq 0) {
                ...
                Write-Host "psql -h $DB_HOST -U $DB_USER -d $DB_NAME -f $migrationFile" -ForegroundColor Cyan
                exit 1
            }
        } else {
            Write-Host "⚠️  psql not found!" -ForegroundColor Yellow
            Write-Host "Install PostgreSQL client or run migration manually:" -ForegroundColor Yellow
            Write-Host "psql -h $DB_HOST -U $DB_USER -d $DB_NAME -f $migrationFile" -ForegroundColor Cyan
            Write-Host ""
