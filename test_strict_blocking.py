...
class StrictSessionManager:
    ...
    def login(self, license_key: str, device_fp: str) -> tuple[bool, str]:
        """
        STRICT BLOCKING: If session exists with heartbeat < 120s, BLOCK ALL
...
def test_strict_blocking_same_device():
    ...
    # Try at different time intervals
    for seconds in [10, 30, 60, 90, 119]:
    ...
    # At 120s, should be allowed
    mgr.sessions[license_key] = (device_fp, datetime.now() - timedelta(seconds=120))
    print(f"\n4. Try again (heartbeat 120s ago - EXPIRED)")
    success, msg = mgr.login(license_key, device_fp)
    print(f"   {msg}")
    assert success, "Should be ALLOWED at 120s (expired)"
    
    print("\n" + "="*70)
    print("✅ PASS: Same device strictly blocked until 120s timeout")
    print("="*70)


def test_strict_blocking_different_device():
    """
    Test: Different device CANNOT login if session exists (any age < 120s)
    """
    print("\n" + "="*70)
    ...
    # At 120s, should be allowed
    mgr.sessions[license_key] = (device_a_fp, datetime.now() - timedelta(seconds=120))
    print(f"\n4. Device B tries again (Device A heartbeat 120s ago - EXPIRED)")
    success, msg = mgr.login(license_key, device_b_fp)
    print(f"   {msg}")
    assert success, "Should be ALLOWED at 120s (expired)"
    
    print("\n" + "="*70)
    print("✅ PASS: Different device strictly blocked until 120s timeout")
    print("="*70)
...
def test_no_exceptions_whatsoever():
    ...
    print(f"\n4. Bot running (heartbeat 60s ago)")
    mgr.sessions[license_key] = (device_fp, datetime.now() - timedelta(seconds=60))
    success, msg = mgr.login(license_key, device_fp)
    print(f"   {msg}")
    assert not success, "Should be BLOCKED at 60s (no crash recovery grace)"
    
    print("\n" + "="*70)
    print("✅ PASS: Absolutely NO exceptions - all logins blocked until 120s")
    print("="*70)
...
def main():
    ...
    try:
        ...
        print("="*70)
        print("\n✅ Implementation:")
        print("   • If session exists (heartbeat < 120s): BLOCK ALL")
        ...
        print("   • Doesn't matter how long (as long as < 120s)")
        print("   • Doesn't matter same or different device")
        print("   • Only exception: session fully expired (>= 120s)")
        print("="*70)
        return 0
